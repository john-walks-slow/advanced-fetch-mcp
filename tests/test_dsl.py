...
class DSLTests(unittest.TestCase):
    ...
    def test_evaluate_js_is_exclusive(self):
        with self.assertRaises(ValidationError):
            AdvancedFetchParams(
                url="https://example.com", evaluateJS="return 1", prompt="x"
            )
        with self.assertRaises(ValidationError):
            AdvancedFetchParams(
                url="https://example.com", evaluateJS="return 1", find_in_page="x"
            )

    def test_evaluate_js_requires_dynamic(self):
        with self.assertRaises(ValidationError):
            AdvancedFetchParams(
                url="https://example.com", mode="static", evaluateJS="return 1"
            )
