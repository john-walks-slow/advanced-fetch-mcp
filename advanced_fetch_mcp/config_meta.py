...
ENV_VAR_SPECS: tuple[EnvVarSpec, ...] = (
    ...
    EnvVarSpec("MAX_LINKS_COUNT", "30", "extraction", "`links` 操作最多返回多少条链接。", "Maximum number of links returned by `links` operation."),
    EnvVarSpec(
        "SCHEMA_LANGUAGE",
        "zh",
        "extraction",
    ...
    EnvVarSpec(
        ...
        "proxy",
        "static 模式是否启用代理。",
        "Whether proxy is enabled for static fetch mode.",
    ),
    EnvVarSpec(
        "ENABLE_DYNAMIC_PROXY",
        "false",
        "proxy",
        "dynamic 模式（浏览器）是否启用代理。",
        "Whether proxy is enabled for dynamic (browser) fetch mode.",
    ),
