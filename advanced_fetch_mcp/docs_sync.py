...
SECTION_LABELS = {
    "zh": {
        ...
        "max_length_scope_desc": "对 `view`、`find`、`sampling`、`eval` 均生效，截断最终返回内容。",
        "cursor_scope": "`render.cursor` 作用域",
        "cursor_scope_desc": "仅对 `view`、`find` 有效。用于从上次返回的 `next_cursor` 位置继续读取或搜索。",
    ...
    "en": {
        ...
        "max_length_scope_desc": "Applies to `view`, `find`, `sampling`, and `eval`, truncating the final returned content.",
        "cursor_scope": "`render.cursor` scope",
        "cursor_scope_desc": "Only valid for `view` and `find`. Used to continue reading or searching from a previous `next_cursor` position.",
