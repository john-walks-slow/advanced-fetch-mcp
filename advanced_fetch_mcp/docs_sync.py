...
SECTION_LABELS = {
    "zh": {
        ...
        "elicit_mode": "`elicit` 模式限制",
        "elicit_mode_desc": "`operation=\"elicit\"` 时，`fetch.mode` 必须为 `\"dynamic\"`。",
    ...
    "en": {
        ...
        "elicit_mode": "`elicit` mode restriction",
        "elicit_mode_desc": "When `operation=\"elicit\"`, `fetch.mode` must be `\"dynamic\"`.",
        ...
        "cursor_scope_desc": "Valid for both `view` and `find`. Used to continue reading from a previous `next_cursor` position.",
...
def render_readme_schema_section(lang: str) -> str:
    ...
    lines.extend(
        [
            ...
            f"| {labels['elicit_mode']} | {labels['elicit_mode_desc']} |",
