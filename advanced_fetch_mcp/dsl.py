...
class AdvancedFetchParams(BaseModel):
    ...
    timeout: Optional[float] = Field(
        default=None,
        ge=0.1,
        description="抓取超时秒数，超时后返回当前已加载内容。",
    )
    markdownify: bool = Field(
        default=True,
        description="是否转换成 Markdown；为 false 时返回原始 HTML。",
    )
    scope: Scope = Field(
        default="content",
    ...
    selector: Optional[str] = Field(
        default=None,
        description="在基础范围内，再用 CSS selector 选出更小的子区域。",
    )
    strip: list[str] = Field(
        default_factory=list,
        description="在当前范围内，按这些 CSS selector 剔除节点。",
    )
    ...
    max_length: int = Field(
        ...
        ge=1,
        description="文本结果长度上限。",
    )
    find_in_page: Optional[str] = Field(
        default=None,
        description="搜索关键词。返回 matches 列表，适合在长页面中定位关键部分。",
    )
    find_with_regex: bool = Field(
        default=False,
